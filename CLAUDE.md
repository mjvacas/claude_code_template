...
- **docs/token-awareness.md**: token cost of the context system — cost classes (`free-local`/`on-use`/`session`/`per-prompt`), the per-session context tax, and a model-routing heuristic. Loaded on demand, not auto-loaded.
