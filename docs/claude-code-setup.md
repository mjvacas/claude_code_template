...
| `statusline.sh` | Status line: model · dir · git branch, plus live context %, 5h rate-limit usage (subscription plans), and an estimated session cost `~$` (pay-as-you-go only — it's notional on a subscription, so the line hides it there and the 5h limit carries the real signal). | `free-local` | Optional — remove the `statusLine` key in `settings.json` to disable. |
