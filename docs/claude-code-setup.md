...
| `statusline.sh` | Status line: model · dir · git branch, plus live context %, session cost, and (subscription plans) 5h rate-limit usage when the event carries them. | `free-local` | Optional — remove the `statusLine` key in `settings.json` to disable. |
